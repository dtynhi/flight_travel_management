...
class FlightTicketClass(db.Model):
    ...
    ticket_class_id = db.Column(
        db.Integer,
        db.ForeignKey("tbl_ticket_classes.id"),
        primary_key=True,
        nullable=False,
    )
    flight_id = db.Column(
        db.Integer, db.ForeignKey("tbl_flights.id"), primary_key=True, nullable=False
    )
    total_seats = db.Column(db.Integer, nullable=False)
    available_seats = db.Column(db.Integer, nullable=False)
    ticket_price = db.Column(db.Numeric(15, 2), nullable=True)  # Cho phép nullable
    ...
    # Relationships
    ticket_class = db.relationship(
        "TicketClass", backref="flight_ticket_classes", lazy="select"
    )
    ...
    def __repr__(self):
        return (
            f"<FlightTicketClass Flight:{self.flight_id} Class:{self.ticket_class_id}>"
        )
